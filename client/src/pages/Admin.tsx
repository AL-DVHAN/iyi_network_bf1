  ...
  if (!isAuthenticated) {
    return (
      <div className="container py-8 text-center">
        ...
        <a href={getLoginUrl()} className="mt-3 inline-block text-sm no-underline" style={{ color: "oklch(0.72 0.14 75)" }}>Giriş Yap</a>
      </div>
    );
  }
    ...
    <div className="container py-8">
      <div className="flex items-center gap-3 mb-6">
        <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ background: "oklch(0.72 0.14 75 / 0.15)" }}>
          <Shield size={20} style={{ color: "oklch(0.72 0.14 75)" }} />
        </div>
        <div>
      ...
      {/* Tabs */}
      <div className="flex gap-1 mb-6 border-b" style={{ borderColor: "oklch(0.25 0.02 60)" }}>
        {TABS.map(t => {
          const Icon = t.icon;
          return (
            <button key={t.key} onClick={() => setTab(t.key)} className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium border-b-2 transition-all" style={{
              borderColor: tab === t.key ? "oklch(0.72 0.14 75)" : "transparent",
        ...
        <div>
          ...
          <div className="space-y-3">
            {feedbackQuery.data?.map(fb => (
              <div key={fb.id} className="rounded-lg border p-4" style={{ background: "oklch(0.16 0.015 58)", borderColor: "oklch(0.25 0.02 60)" }}>
          ...
          <div className="rounded-lg border p-4" style={{ background: "oklch(0.16 0.015 58)", borderColor: "oklch(0.25 0.02 60)" }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                ...
                <input value={banForm.eaUsername} onChange={e => setBanForm(p => ({ ...p, eaUsername: e.target.value }))} className="w-full h-9 px-3 text-sm rounded border outline-none" style={{ background: "oklch(0.18 0.01 60)", borderColor: "oklch(0.28 0.03 60)", color: "oklch(0.92 0.02 80)" }} />
              </div>
              <div>
                <label className="text-xs text-muted-foreground block mb-1">Ban Türü</label>
                <select value={banForm.banType} onChange={e => setBanForm(p => ({ ...p, banType: e.target.value as typeof p.banType }))} className="w-full h-9 px-3 text-sm rounded border outline-none" style={{ background: "oklch(0.18 0.01 60)", borderColor: "oklch(0.28 0.03 60)", color: "oklch(0.92 0.02 80)" }}>
              ...
              <div className="md:col-span-2">
                ...
                <input value={banForm.reason} onChange={e => setBanForm(p => ({ ...p, reason: e.target.value }))} className="w-full h-9 px-3 text-sm rounded border outline-none" style={{ background: "oklch(0.18 0.01 60)", borderColor: "oklch(0.28 0.03 60)", color: "oklch(0.92 0.02 80)" }} />
              </div>
              {banForm.banType === "temporary" && (
                <div>
                  <label className="text-xs text-muted-foreground block mb-1">Bitiş Tarihi</label>
                  <input type="date" value={banForm.expiresAt} onChange={e => setBanForm(p => ({ ...p, expiresAt: e.target.value }))} className="w-full h-9 px-3 text-sm rounded border outline-none" style={{ background: "oklch(0.18 0.01 60)", borderColor: "oklch(0.28 0.03 60)", color: "oklch(0.92 0.02 80)" }} />
                </div>
              )}
            </div>
      ...
      {tab === "leaderboard" && (
        <div className="rounded-lg border p-4" style={{ background: "oklch(0.16 0.015 58)", borderColor: "oklch(0.25 0.02 60)" }}>
          ...
          <button onClick={() => addLbMutation.mutate(lbForm)} disabled={!lbForm.eaUsername || addLbMutation.isPending} className="mt-3 px-4 py-2 rounded text-sm font-semibold" style={{ background: "oklch(0.72 0.14 75)", color: "oklch(0.10 0.01 60)" }}>
        ...
        <div>
          ...
          <div className="space-y-3">
            {appealsQuery.data?.map(appeal => (
              <div key={appeal.id} className="rounded-lg border p-4" style={{ background: "oklch(0.16 0.015 58)", borderColor: "oklch(0.25 0.02 60)" }}>
                <div className="flex items-start justify-between gap-2 mb-2">
